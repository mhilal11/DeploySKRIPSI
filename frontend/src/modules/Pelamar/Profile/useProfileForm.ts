...
const ensureEducationId = (item: Education): Education => ({
  ...item,
  id:
...
const ensureExperienceId = (item: Experience): Experience => ({
  ...item,
  id:
    typeof item.id === "string" && item.id.trim() !== ""
      ? item.id
...
const ensureCertificationId = (item: Certification): Certification => ({
  ...item,
  id:
    typeof item.id === "string" && item.id.trim() !== ""
      ? item.id
...
export function useProfileForm(profile: ApplicantProfilePayload) {
  const initialData: ApplicantProfileForm = {
    ...
    educations:
      profile.educations.length > 0
        ? profile.educations.map((item) => ensureEducationId({ ...item }))
        : [createEmptyEducation()],
    experiences:
      profile.experiences.length > 0
        ? profile.experiences.map((item) =>
            ensureExperienceId(normalizeExperience(item)),
          )
        : [],
    certifications:
      profile.certifications?.length > 0
        ? profile.certifications.map((item) =>
            ensureCertificationId({ ...item, file: null }),
          )
        : [],
    profile_photo: null,
  };
  ...
  const [submittingSection, setSubmittingSection] = useState<SectionKey | null>(
    null,
  );
  ...
  const addEducation = () => {
    form.setData({
      ...form.data,
      educations: [...form.data.educations, createEmptyEducation()],
    });
  };
  ...
  const addExperience = () => {
    form.setData({
      ...form.data,
      experiences: [...form.data.experiences, createEmptyExperience()],
    });
  };
  ...
  const addCertification = () => {
    form.setData({
      ...form.data,
      certifications: [...form.data.certifications, createEmptyCertification()],
    });
  };
  ...
  const submitSection = (section: SectionKey) => {
    if (section === "experience") {
      if (form.data.experiences.length === 0) {
        form.setError(
          "experiences",
          "Minimal 1 pengalaman kerja/magang wajib diisi.",
        );
        toast.error("Gagal Menyimpan ", {
          description:
            "Tambahkan minimal satu pengalaman kerja/magang sebelum menyimpan.",
          duration: 4000,
        });
        return;
      }
    ...
    form.post(route("pelamar.profile.update"), {
      ...
      onError: (serverErrors) => {
        const errorMessages = {
          ...
          experience:
            "Periksa kembali semua field wajib pada data pengalaman kerja/magang.",
  ...
  const getEducationError = (index: number, field: RequiredEducationField) =>
