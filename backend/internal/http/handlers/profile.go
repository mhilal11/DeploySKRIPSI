...
func UpdateProfile(c *gin.Context) {
	...
	if err := c.ShouldBind(&req); err != nil {
		ValidationErrors(c, FieldErrors{"name": "Nama wajib diisi.", "email": "Email wajib diisi."})
		return
	}
...
func UpdatePassword(c *gin.Context) {
	...
	if bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(req.CurrentPassword)) != nil {
		ValidationErrors(c, FieldErrors{"current_password": "Password saat ini salah."})
		return
	}
...
func DeleteProfile(c *gin.Context) {
	...
	if err != nil {
		JSONError(c, http.StatusInternalServerError, "Gagal menghapus akun")
		return
	}
